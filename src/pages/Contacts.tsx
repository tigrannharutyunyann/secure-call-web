...
const securityBadges = [
  { label: "E2E secured", icon: ShieldCheckIcon },
  { label: "Device-locked keys", icon: KeyIcon },
  { label: "Session transparency", icon: EyeIcon },
];
  ...
  return (
    <AppLayout>
      <div className="flex-1 min-h-screen bg-gray-50/60 dark:bg-[#0b0b0b]">
        {!selected ? <EmptyState /> : <ContactPanel contactId={selected.id} />}
      </div>
    </AppLayout>
  );
}
  ...
  return (
    <div className="h-full min-h-screen flex items-center justify-center p-6">
      <div className="max-w-lg w-full panel bg-[#0f1f2b] text-white border border-accent/20 shadow-2xl rounded-2xl p-8 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold">Start a secure call</h2>
        <p className="text-white/80 text-sm leading-relaxed">
          Select a contact on the left to see trust status and start an encrypted session.
        </p>
      </div>
    </div>
  );
...
function ContactPanel({ contactId }: { contactId: string }) {
  ...
  return (
    <div className="p-6 lg:p-10 flex flex-col gap-6">
      <div className="panel bg-[#0f1f2b] text-white border border-accent/20 shadow-xl rounded-2xl p-6 sm:p-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="relative">
            <img
              src={contact.avatar}
              alt={contact.name}
              className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl border border-accent/30 object-cover"
            />
            <span className="absolute -right-1 -bottom-1 w-3 h-3 rounded-full bg-emerald-400 ring-2 ring-[#0f1f2b]" />
          </div>
          <div className="space-y-1">
            <h2 className="text-2xl sm:text-3xl font-semibold">{contact.name}</h2>
            <p className="text-white/70 text-sm sm:text-base">{contact.status}</p>
            <p className="text-white/60 text-sm">Key pinned • Secure</p>
          </div>
          <div className="sm:ml-auto flex gap-3">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg border border-white/20 text-white/80 hover:text-white hover:border-accent/50 hover:bg-accent/10 transition"
            >
              View security
            </button>
          </div>
            ...
            <button
              ...
              onClick={() =>
                setBadgeModal(
                  label === "E2E secured"
                    ? "e2e"
                    : label === "Device-locked keys"
                    ? "device"
                    : "transparency"
                )
              }
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/15 bg-white/5 text-white/85 text-sm hover:border-accent/40 hover:bg-accent/10 transition"
            >
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
            ...
            <p className="text-white/70 text-sm leading-relaxed">
              End-to-end encrypted audio with per-device keys. Session transparency warns about key changes or new devices.
            </p>
            <p className="text-white/60 text-xs">Key fingerprint: {fingerprint}</p>
          </div>

          <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
            <div className="flex items-center gap-2">
              <PhoneLockIcon className="w-5 h-5 text-accent" />
              <p className="font-semibold text-white">Key fingerprints</p>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between bg-black/10 rounded-lg px-3 py-2">
                <span className="text-white/70">My key fingerprint</span>
                <span className="font-mono text-white/90">AA:BB:CC:DD:…</span>
              </div>
              <div className="flex items-center justify-between bg-black/10 rounded-lg px-3 py-2">
                <span className="text-white/70">Contact’s fingerprint</span>
                <span className="font-mono text-white/90">FF:EE:DD:CC:…</span>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setVerifyModal(true)}
              className="w-full mt-2 px-4 py-3 rounded-lg border border-accent/40 bg-accent/10 text-white hover:bg-accent/20 transition text-sm"
            >
              Verify keys
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="font-semibold text-white">Audio setup</p>
            </div>
            <div className="space-y-3">
              <div className="space-y-1">
                <p className="text-sm text-white/80">Microphone</p>
                <div className="relative">
                  <div
                    onClick={() => setIsMicOpen((prev) => !prev)}
                    className="flex items-center justify-between w-full px-4 py-3 rounded-lg bg-black/10 border border-white/10 cursor-pointer hover:bg-black/20 transition"
                  >
                  ...
                  {isMicOpen && (
                    <div className="absolute z-10 mt-2 w-full rounded-lg border border-white/10 bg-[#0f1f2b]/95 backdrop-blur-sm shadow-xl">
                      {micOptions.length === 0 && (
                        <div className="px-4 py-2 text-sm text-white/70">No inputs found</div>
                      )}
                      {micOptions.map((dev) => (
                        <button
              ...
              <div className="space-y-1">
                <p className="text-sm text-white/80">Speaker output</p>
                <div className="relative">
                  <div
                  ...
                  {isSpeakerOpen && (
                    <div className="absolute z-10 mt-2 w-full rounded-lg border border-white/10 bg-[#0f1f2b]/95 backdrop-blur-sm shadow-xl">
                      {speakerOptions.length === 0 && (
                        <div className="px-4 py-2 text-sm text-white/70">No outputs found</div>
                      )}
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-3 rounded-lg border border-accent/30 bg-accent/10 text-white hover:bg-accent/20 transition text-sm"
              >
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="font-semibold text-white">Last calls</p>
            ...
            <div className="space-y-2">
              {mockCalls.map((call) => (
                <button
                  ...
                  className="w-full flex items-center justify-between rounded-lg border border-white/5 bg-white/5 px-3 py-2 hover:border-accent/40 hover:bg-accent/10 transition"
                >
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-emerald-400" />
                    <p className="text-white text-sm">{call.title}</p>
                  </div>
                  <p className="text-white/60 text-xs">{call.time}</p>
                </button>
              ))}
            </div>
          </div>
